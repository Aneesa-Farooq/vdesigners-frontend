      ...
      <div class="flex flex-col lg:flex-row lg:gap-6 w-full p-5 bg-white rounded-md mb-3">
        ...
        <div class="self-center">
          <div class="flex items-center gap-2 lg:gap-5 mb-2">
            ...
            <p class="flex items-center text-base lg:text-lg font-medium text-mediumPink">{{ post.designerId }}</p>
            <p :class="`${(post.status == 'pending') ? 'blink_me' : 'bg-Green'}`"
              class=" flex items-center justify-center text-[10px] lg:text-xs tracking-wide font-medium text-white p-1 lg:px-3 text-center rounded-xl w-fit h-fit">
              {{ post.status }}</p>
          </div>
